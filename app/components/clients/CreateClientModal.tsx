...
import type { ClientFormData } from '../../types/auth';
  ...
  const onSubmit = async (values: CreateClientFormValues) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        const message = response.error || 'Error al crear cliente';
        setError(message);
        showError(message);
      }
    } catch (err) {
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label htmlFor="create-client-name" className="block text-sm font-medium text-gray-700 mb-1">Razon social *</label>
                <Input id="create-client-name" {...register('name')} maxLength={100} className={cn(errors.name && 'border-red-300')} />
                {errors.name && <p className="mt-1 text-sm text-red-600">{errors.name.message}</p>}
              </div>

              <div>
                <label htmlFor="create-client-type" className="block text-sm font-medium text-gray-700 mb-1">Tipo *</label>
                <select
              ...
              <div>
                <label htmlFor="create-client-document-number" className="block text-sm font-medium text-gray-700 mb-1">NIT *</label>
                <Input id="create-client-document-number" {...register('documentNumber')} maxLength={20} className={cn(errors.documentNumber && 'border-red-300')} />
                {errors.documentNumber && <p className="mt-1 text-sm text-red-600">{errors.documentNumber.message}</p>}
              </div>

              <div className="md:col-span-2">
                <label htmlFor="create-client-address" className="block text-sm font-medium text-gray-700 mb-1">Direccion *</label>
                <Input id="create-client-address" {...register('address')} maxLength={256} className={cn(errors.address && 'border-red-300')} />
                {errors.address && <p className="mt-1 text-sm text-red-600">{errors.address.message}</p>}
              </div>

              <div>
                <label htmlFor="create-client-city" className="block text-sm font-medium text-gray-700 mb-1">Ciudad *</label>
                <Input id="create-client-city" {...register('city')} maxLength={50} className={cn(errors.city && 'border-red-300')} />
                {errors.city && <p className="mt-1 text-sm text-red-600">{errors.city.message}</p>}
              </div>

              <div>
                <label htmlFor="create-client-phone" className="block text-sm font-medium text-gray-700 mb-1">Telefono *</label>
                <Input id="create-client-phone" {...register('phone')} maxLength={10} className={cn(errors.phone && 'border-red-300')} />
                {errors.phone && <p className="mt-1 text-sm text-red-600">{errors.phone.message}</p>}
              </div>

              <div className="md:col-span-2">
                <label htmlFor="create-client-email" className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                <Input id="create-client-email" type="email" {...register('email')} maxLength={70} className={cn(errors.email && 'border-red-300')} />
              ...
              <div className="md:col-span-2 flex items-center">
                ...
                />
                <label htmlFor="create-client-active" className="ml-2 text-sm text-gray-700">Activo</label>
              </div>
            </div>
