...
import type { InventoryFormData, InventoryRecord, Product } from '../../types/auth';
...
export function EditInventoryModal({ isOpen, onClose, onSuccess, record }: EditInventoryModalProps) {
  ...
  const defaultValues = useMemo<EditInventoryFormValues>(() => ({
    product: record.product?.id || '',
    quantity: record.quantity,
    lotNumber: record.lotNumber,
    expirationDate: (record.expirationDate || '').replace(/\//g, '-')
  }), [record]);
  ...
  const lastSelectedProductIdRef = useRef<string | null>(null);

  useEffect(() => {
    if (isOpen) {
      reset(defaultValues);
      lastSelectedProductIdRef.current = defaultValues.product;
      setError(null);
      loadProducts();
    }
  ...
  useEffect(() => {
    ...
    if (lastSelectedProductIdRef.current === selectedProduct.id) {
      return;
    }
  ...
  useEffect(() => {
    if (products.length > 0 && defaultValues.product) {
      const exists = products.some((p) => p.id === defaultValues.product);
      if (exists) {
        setValue('product', defaultValues.product);
      }
    }
  }, [products, defaultValues.product, setValue]);
  ...
  const onSubmit = async (values: EditInventoryFormValues) => {
    ...
    try {
      ...
      const response = await inventoryService.update(record.id, payload);

      if (response.success) {
        success('Registro de inventario actualizado exitosamente');
        onSuccess();
        onClose();
      } else {
        const message = response.error || 'Error al actualizar registro de inventario';
        setError(message);
        showError(message);
      }
    } catch (err) {
      ...
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="relative inline-block w-full max-w-2xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-lg z-50">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
            <h3 className="text-lg font-medium text-gray-900">Editar Registro de Inventario</h3>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-6 py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                <label htmlFor="inventory-edit-product" className="block text-sm font-medium text-gray-700 mb-1">Producto *</label>
                <select
                  id="inventory-edit-product"
                  {...register('product')}
                  className={cn(
              ...
              <div>
                <label htmlFor="inventory-edit-quantity" className="block text-sm font-medium text-gray-700 mb-1">Cantidad (Unidad(es)) *</label>
                <Input
              ...
              <div>
                <label htmlFor="inventory-edit-expiration" className="block text-sm font-medium text-gray-700 mb-1">Fecha de vencimiento *</label>
                <Input
                  id="inventory-edit-expiration"
                  type="date"
              ...
              <div className="md:col-span-2">
                <label htmlFor="inventory-edit-lot-number" className="block text-sm font-medium text-gray-700 mb-1">Lote *</label>
                <Input
                  id="inventory-edit-lot-number"
                  type="text"
